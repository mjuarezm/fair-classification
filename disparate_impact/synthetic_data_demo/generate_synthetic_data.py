...
def generate_synthetic_data(n_samples=1000, plot_data=False):
    ...
    # shuffle the data
    perm = np.arange(0, n_samples*2).astype(int)
    shuffle(perm)
    X = X[perm]
    y = y[perm]
    ...
    x_control = np.array(x_control).astype(int)
    ...
    if plot_data:
        ...
        X_s_0 = x_draw[x_control_draw == 0]
        X_s_1 = x_draw[x_control_draw == 1]
        y_s_0 = y_draw[x_control_draw == 0]
        y_s_1 = y_draw[x_control_draw == 1]
        plt.scatter(X_s_0[y_s_0 ==1][:, 0], X_s_0[y_s_0 ==1][:, 1], color='green', marker='x', s=30, linewidth=1.5, label= "Prot. +ve")
        plt.scatter(X_s_0[y_s_0==-1][:, 0], X_s_0[y_s_0==-1][:, 1], color='red', marker='x', s=30, linewidth=1.5, label = "Prot. -ve")
        plt.scatter(X_s_1[y_s_1 ==1][:, 0], X_s_1[y_s_1 ==1][:, 1], color='green', marker='o', facecolors='none', s=30, label = "Non-prot. +ve")
        plt.scatter(X_s_1[y_s_1==-1][:, 0], X_s_1[y_s_1==-1][:, 1], color='red', marker='o', facecolors='none', s=30, label = "Non-prot. -ve")
    ...
    c_control = x_control.copy()
    x_control = {"s1": x_control} # all the sensitive features are stored in a dictionary
    return X[c_control==1], y[c_control==1], X[c_control==0], y[c_control==0], X, y, x_control
